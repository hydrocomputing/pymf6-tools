...
from pymf6_tools.make_model import get_simulation
...
def show_heads(
    ...
    plot = pmv.plot_vector(qx, qy, normalize=True, color='white')
...
def show_concentration(
    ...
    if show_rivers:
        plot = pmv.plot_bc(
            package=gwf.get_package('riv-1'), plotAll=True, kper=1
        )
    if show_contours:
        pmv.contour_array(
            conc,
...
def show_well_head(
    ...
    if one_limit or limit_range:
        ax.plot(x, y1, color='red', linestyle=':', label=text)
    if limit_range:
    ...
    ax.plot(*x_stresses, color='lightblue', linestyle=':')
    ax.legend(loc=(1.1, 0))
    return ax
...
def contour_well_heads(
    ...
    plot = pmv.plot_vector(qx, qy, normalize=True, color='grey')
    plot.axes.set_xlabel('x (m)')
    plot.axes.set_ylabel('y (m)')
    plot.axes.set_title(title)
